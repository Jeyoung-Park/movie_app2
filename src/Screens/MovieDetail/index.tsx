...
import BigCatalog from '~/Compoonents/BigCatalog';
...
//영화 리스트 컴포넌트에서 영화를 선택하면 매개 변수로 id 값을 받아옴
const MovieDetail=({navigation}:Props)=>{
    const[data, setData]=useState<IMovieDetail>();
    // 두 번째 매개변수가 빈 배열이므로, 화면에 표시될 처음에만 호출
    useEffect(()=>{
        const id=navigation.getParam('id'); //매개변수로 넘어온 값 받기 (intent와 비슷한 역할)
        fetch(`http://yts.lt.api/v2/movie_details.json?movie_id=${id}&with_images=true&with_cast=true`)
        .then(response=>response.json())
        .then(json=>{
            console.log(json);
            setData(json.data.movie); //data에 api에서 받아온 json 파일을 state로 넣어줌
        })
        .catch(error=>{
            console.log(error);
        });
